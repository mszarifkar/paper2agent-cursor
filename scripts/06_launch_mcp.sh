...
# Note: This script calls 'claude' CLI which is optional for Claude Code integration
# For Cursor Composer workflow, this script is optional and can be skipped
if [[ $# -lt 2 ]]; then
...
# Note: This calls 'claude' CLI which is optional for Cursor workflow
if command -v claude &> /dev/null; then
  claude || echo "06: claude code client exited" >&2
else
  echo "06: 'claude' CLI not found - skipping launch (optional for Cursor workflow)" >&2
fi
